function solution = solveCobraLP_par(LPproblem, thesolver,varargin)
...
% Gonghua Li         10/11/17 Added support for cplex 
...
switch solver
    ...
    case 'cplex'
        ...
        if sum(csenseE) == length(b) 
            [x, f, exitflag, output] = cplexlp(osense*c',[],[],A,b,lb,ub,[],options);
        else
            Aineq = A(~csenseE,:);
            bineq = b(~csenseE);
            Aeq = A(csenseE,:);
            beq = b(csenseE,:);
            [x, f, exitflag, output] = cplexlp(osense*c',Aineq,bineq,Aeq,beq,lb,ub,[],options);
        end
        f = f*osense;

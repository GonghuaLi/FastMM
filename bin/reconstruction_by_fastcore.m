function out = reconstruction_by_fastcore(consmodel,expr_matrix_file,cutoff)
...
for i  = 1:size(expr,2)
    ...
    out_rxns = fastcore( core_rxns, modela, 1e-4);
    out(:,i) = ismember(cc,out_rxns);
end
header = ['rxns',sampleNames];
writetxt([header;[modela.rxns,num2cellstr(out)]],'./out/reconstructed_model.txt','\t');
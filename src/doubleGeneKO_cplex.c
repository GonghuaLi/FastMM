...
{ 
	...
	int error, numMets,numRxns, status,*coltype,is_constraint=0,is_objf=0,is_effective_file = 0;
		...
		for (j=i+1;j<num_gen ;j++ )
		{
			...
			}else
			{
				...
				for (s = 0;s<num_obj ;s++ )
				{
					...
					//glp_set_obj_coef(P,obj[s],type);
				    for (k = 0;k<n ;k++ ) {
						error = CPXchgbds (env, model, 1, changedRxns+k, lu_b, tmpval_0);
				    ...
				    for (k = 0;k<n ;k++ ) {	
						...
						error = CPXchgbds (env, model, 1, changedRxns+k, lu_l, lb+changedRxns[k]);
					    error = CPXchgbds (env, model, 1, changedRxns+k, lu_u, ub+changedRxns[k]);

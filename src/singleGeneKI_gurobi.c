...
{ 
	...
	int numRxns, error, numMets,nmets,nrxns,*coltype,is_constraint=0,is_objf=0;
			...
			for (s = 0;s<num_obj ;s++ )
			{
			   ...
			   // }
				...
				for (j = 0;j<n ;j++ ) {
					...
					error = GRBsetdblattrelement(model, GRB_DBL_ATTR_LB,changedRxns[j],relaxlb);
					error = GRBsetdblattrelement(model, GRB_DBL_ATTR_UB,changedRxns[j],relaxub);

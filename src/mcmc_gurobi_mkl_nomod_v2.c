...
int main(int   argc, char **argv)
{
    ...
    npoints=str2int(argv[2]);//npoints=str2int(argv[3]);
	nsteps=str2int(argv[3]);//nsteps=str2int(argv[4]);
	strcpy(outfile,argv[4]);//strcpy(outfile,argv[5]);
...
//	}
    ...
    double maxMinTol = 1e-9; // Minimum allowed distance to the closest constraint
    double uTol = 1e-9; // Ignore directions where u is really small
    double dTol = 1e-14; // Project out of directions that are too close to the boundary
	//double maxMinTol = 1e-5; // Minimum allowed distance to the closest constraint
    //double uTol = 1e-5; // Ignore directions where u is really small
    //double dTol = 1e-6; // Project out of directions that are too close to the boundary
...
//			}
			...
			//for (k=0;k<nrxns ;k++ )
			//{
			//	if (prevPoint[k]-UB[k]>1e-8 | prevPoint[k]-LB[k] <-1e-8)
			//	{
			//		is_continue = 1;
			//		break;
			//	}

...
int help()
{
	printf("\n---------------------FastMM:doubleGeneKO-----------------------\n");
	printf("\n---------------------  Version 0.01   -----------------------\n");
    printf("        C version of double gene  knockout analysis   \n");
	...
	printf("        -e effective_rxnsfile (optional,default no use this file)\n");

	printf("eg: doubleGeneKO -m cobramodel -t max -o outputfile\n\n");
...
{ 
	...
	int error, numMets,numRxns,*coltype,is_constraint=0,is_objf=0,is_effective_file = 0;
	double duration,type;
	double *lb,*ub;
	char buf[2048],objfile[300],outputfile[300],optimizeType[100],constraintfile[300],effectfile[300],gene_rxns_file[300];
	char **rules;
	...
	for (i=1;i<argc-1 ;i++ )
	{
		...
		else if (strcmp(argv[i],"-g")==0) {strcpy(gene_rxns_file,argv[i+1]);}
	...
	//read objective function file and input_effectRxns
    ...
    printf("FastMM:doubleGeneKO: Number of active genes is %d\n",num_effective_genes);
	...
	printf("FastMM:doubleGeneKO:  Start single gene deletion....\n");
	...
	printf("FastMM:doubleGeneKO:  Single gene deletion: completed!\n\n");

	printf("FastMM:doubleGeneKO:  Start Double gene deletion\n");
	...
	int numLP=0;
	int totalLP = num_gen*(num_gen-1)/2;
	int pass = totalLP/100;
	for (i = 0;i<num_gen ;i++ )
	{
		//printf("%dth of %d\n",i,num_gen);
		for (j=i+1;j<num_gen ;j++ )
		{
			...
			}else
			{
				...
				for (s = 0;s<num_obj ;s++ )
				{
				    ...
				    //n = get_changed_effectiveRxns_by_gens(rules,numRxns,is_expr,changedRxns,iseffectiveRxns,&effectiveStat);
				    if (effectiveStat<1)
				    {
					    //is_expr[i] = 1; is_expr[j] = 1;
					    fprintf(fout,"\t%.6f",val_no_delet[s]);
    ...
    printf( "\nRuning whole genome double gene deletion spend: %f seconds\n", duration );
	return 0;
}

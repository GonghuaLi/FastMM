...
int main(int   argc, char **argv)
{
    ...
    double uTol = 1e-10; // Ignore directions where u is really small, default
    double dTol = 1e-14; // Project out of directions that are too close to the boundary,default
    ...
    if (argc>7)
    {
		...
		uTol = str2double(argv[7]);
		dTol = str2double(argv[8]);
	...
	for (i=0;i<npoints ;i++ )
	{
		...
		while(stepCount<nsteps)
		{
			...
			if ((totalStepCount+1) % 100 == 0)
			{
				 ...
				 for (k=0;k<nrxns ;k++ )
			     {
			    	 if (prevPoint[k] > UB[k] + 1e-9) {prevPoint[k] = UB[k]; is_continue = 1;}
			    	 if (prevPoint[k] < LB[k] - 1e-9) {prevPoint[k] = LB[k]; is_continue = 1;}
			     }
				//printf("%i\n",totalStepCount);
	...
	//Mat_VarWrite(pmatout,pa1,MAT_COMPRESSION_ZLIB);
